...
<head>
    ...
    <title>Dashboard attach pdf file</title>
</head>
<body>
...
<div class="pageArea">
	...
	$extension = end($temp);
		...
		if ($error > 0){
					...
					if($size > 1000000000) //conditions for the file to be uploaded correctly
					{
                ...
                if($update=mysqli_query($link, "UPDATE books_tb SET cover_image='$img_name' WHERE book_id='$book_id' AND recycle='False'")) {
                    ...
                    <i class='fa-solid fa-check'></i> Cover image has been changed successfully!<br>
	...
	?>
        ...
        <input type="hidden" name="book_id" value="<?php echo"$book_id";?>"  id="book_id" >
                ...
                <button name="image" type="submit">Upload and Save Changes <i class="fa-solid fa-cloud-arrow-up"></i></button>
            </form>
...
while($db_field = mysqli_fetch_assoc($result)){
    ...
    $uploaded_by = $db_field['uploaded_by'];
        ...
        <td><a href='attach_pdf_file.php?book_id=$book_id'><i class='fa-regular fa-file-pdf'></i></a></td>
